...
class update_model extends CI_Model {
	...
	public function emphide($hideempID){
		$this->db->where('empID',$hideempID);
		$this->db->delete('emphistory');
	}
...
*/
	...
	public function uploadedCOE($idno){
		$this->db->set('uploaded_coe', 'uploaded_coe+1', FALSE);
		$this->db->where('idnumber',$idno);
		$this->db->update('users');
	}

	public function delCert($delcertID) {
		$this->db->where('certID',$delcertID);
		$this->db->delete('empcertificates');
	}

	public function deletedCOE($idno) {
		$this->db->set('uploaded_coe', 'uploaded_coe-1', FALSE);
		$this->db->where('idnumber',$idno);
		$this->db->update('users');
	}

	public function validatedCOE($certidnumber){
		$this->db->set('validated_coe', 'validated_coe+1', FALSE);
		$this->db->where('idnumber',$certidnumber);
		$this->db->update('users');
	}

	public function invalidatedCOE($certidnumber){
		$this->db->set('validated_coe', 'validated_coe-1', FALSE);
		$this->db->where('idnumber',$certidnumber);
		$this->db->update('users');
	}

	public function certValidated($certid,$validated){
		$this->db->where('certID',$certid);
		$this->db->update('empcertificates',$validated);
	}

	public function certInvalidated($certid,$invalidated){
		$this->db->where('certID',$certid);
		$this->db->update('empcertificates',$invalidated);
	}
