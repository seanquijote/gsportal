...
if($row->nfPostedBy == $idno) {
	...
	echo '<br><small style="color:#999999;">&nbsp;&nbsp;'.timespan($post_date, $now, $units).' ago</small><br><small id="markEdit" style="font-weight:bold; margin-left:8px; color:#afafaf;">'.$row->nfEdited.'</small></div><p style="margin-left:10px; font-size:16px;">'.$row->nfContent.'</p>';
		if($row->nfStatus == 'reported') {
			echo '</div><div style="padding: 8px; width:100%; background:red; text-align:center"><a style="padding: 10px; border-radius: 0px;color: white; text-decoration:none;"><span class="btn btn-danger glyphicon glyphicon-alert" style="color:white; position:relative;"></span> This post cannot be commented until the administrator has reviewed this post.</a></div></div>';
		} else {
			echo '</div><center><a class="btn btn-primary" style="width:100%; border-radius: 0px;" id="displayText'.$row->newsfeedID.'" href="'.base_url('user/newsfeed/'.$row->nfPostedBy.'/'.$row->newsfeedID).'"><i class="fa fa-comments" aria-hidden="true"></i>&nbsp; View all comments or comment on this post!</a></center></div>';
		}
}
