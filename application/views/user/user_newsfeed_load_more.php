...
if($row->userstatus != 'disabled') {
	if($row->nfStatus == 'reported') {
		...
		echo '</div><a class="btn btn-primary" style="width:100%; border-radius: 0px; text-align:center" id="displayText'.$row->newsfeedID.'" href="'.base_url('user/newsfeed/'.$row->nfPostedBy.'/'.$row->newsfeedID).'"><i class="fa fa-comments" aria-hidden="true"></i>&nbsp; View all comments or comment on this post!</a></div>';
	}
} else {
